...
use util::{
    algebra::{coset::Coset, field::FftField},
...
#[derive(Clone)]
pub struct Verifier<T: FftField> {
    total_round: usize,
...
impl<T: FftField> Verifier<T> {
    pub fn new(
        ...
        Verifier {
            ...
            shuffle_eval: None,
            deep_evals: vec![],
            open_point: (0..total_round).map(|_| T::rand(rng)).collect(),
            step,
        }
    }
